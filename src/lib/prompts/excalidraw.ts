...
### 时序图（强制遵守）
请严格按照以下规则，在 Excalidraw 中绘制一个专业、清晰的时序图。

### **1. 绘制参与者**
*  在画布顶部，用矩形水平排列所有参与者，保持统一大小和间距。
*  为每个参与者矩形设置唯一 ID，例如 “actor_user”、“actor_server”。
*  为不同参与者分配**独立且一致**的配色（示例：蓝 #2196f3、绿 #4caf50、橙 #ff9800、紫 #9c27b0）；矩形填充与对应生命线使用同色系，文字用深色确保可读。

### **2. 绘制生命线**
*  从每个参与者矩形的底部中心开始，向下画一条垂直虚线（type: "line", strokeStyle: "dashed"）。
*  生命线颜色与所属参与者矩形一致，保持清晰可辨。
*  生命线只用 line，不要设置 start/end 绑定，也不要使用 type:"point"。

### **3. 绘制消息箭头（核心规则）**
*  所有消息箭头沿垂直方向自上而下排列，Y 坐标单调递增。
*  箭头的 X 坐标对齐发送方与接收方矩形的中心，左右方向由二者位置决定。
*  在箭头中部放置简洁中文标签；箭头颜色可用中性深灰（如 #444）以突出节点配色。
*  关键绑定：每个箭头必须同时设置 start.id 与 end.id，值为参与者矩形的 ID，缺失任一绑定视为无效；严禁使用 type:"point"。

### **4. 布局与样式**
*  保持相邻消息箭头 80~120 像素的垂直间距，避免箭头交叉。
*  需要标题时，在画布顶部中央放置一个 text 元素。
